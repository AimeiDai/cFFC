...
##############
all = subset(read.table("Nonconserved_Family_Info.txt",stringsAsFactors=F, header=T,sep="\t",quote=""),Species.ID==7227)
...
miRfami = read.table("../2.miR_TF_expression/miR_expression/miR_Family_Info_modified.txt",stringsAsFactors=F, header=T,sep="\t",quote="")

#cons = subset(miRfami,Family.Conservation.==1)
miRfami = unique(subset(miRfami,Family.Conservation.!=1)[,1:2])

rownames(miRfami)=miRfami[,2]
all$miRs = miRfami[all[,1],1]

motifs = read.table("l3_motifs.nonconsmiR.lowThreshold",stringsAsFactors=F)
...
tf_tg = read.table("../1.TF_bindings/W3L_tf_bindings.txt",stringsAsFactors=F)

all_tgs = data.frame(tgs,row.names=tgs)
# expr_mirs = read.table("../2.miR_TF_expression/miR_expression/l3_miRs.cons.low.txt",stringsAsFactors=F)
...
table(all_tgs[,2])
# high-redundant  low-redundant    miR->non-TF  non-redundant          other 
#           3156           5495            353           2036           2386 

write.table(all_tgs,"l3_nonconsmiR.lowThreshold.allTargetType",row.names=F,col.names=T,sep="\t",quote=F)


tf_tg = tf_tg[tf_tg[,3] %in% tgs,]
expressed_tfs = read.table("../2.miR_TF_expression/TF_expression/filtered_L3_TFs.txt",stringsAsFactors=F)
...
save(type,file="l3_nonconsmiR.lowThreshold.redundancy_type.RData")

#load("redundancy_type.RData")
mir_tg$type=type
# x=subset(mir_tg,PCT!="NULL")
# x1=subset(x,type=="high-redundant")
# by(x$PCT,x$type,function(x){median(as.numeric(x))})

write.table(mir_tg,"l3_nonconsmiR.lowThreshold.redundancy",row.names=F,col.names=T,quote=F,sep="\t")
