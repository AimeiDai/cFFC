...
motifs = read.table("embryonic_motifs.nonconsmiR.lowThreshold",stringsAsFactors=F)
...
write.table(all_tgs,"Embryo_nonconsmiR.lowThreshold.allTargetType",row.names=F,col.names=T,sep="\t",quote=F)
...
save(type,file="Embryonic_nonconsmiR.lowThreshold.redundancy_type.RData")
...
write.table(mir_tg,"Embryonic_nonconsmiR.lowThreshold.redundancy",row.names=F,col.names=T,quote=F,sep="\t")
